...
import { FiFileText, FiEdit2, FiTrash2, FiCopy, FiDownload, FiCalendar, FiLayout, FiEye } from 'react-icons/fi';
...
export default function ResumeCard({ resume, onEdit, onDelete, onDuplicate, onGeneratePDF, onPreview, isLoading }) {
    return (
        <div className={styles.card}>
            ...
            <div className={styles.cardFooter}>
                {resume.is_public && onPreview && (
                    <Button variant="outline" size="sm" icon={<FiEye />} onClick={() => onPreview(resume.slug)} disabled={isLoading}>
                        Preview
                    </Button>
                )}
                ...
                <Button variant="outline" size="sm" icon={<FiDownload />} onClick={() => onGeneratePDF(resume.resume_id)} disabled={isLoading}>
                    {resume.is_pdf_generated ? 'Regen PDF' : 'PDF'}
                </Button>
