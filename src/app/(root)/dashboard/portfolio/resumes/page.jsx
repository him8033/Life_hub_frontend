        ...
        <div className={styles.pageContainer}>
            ...
            {resumes.length > 0 ? (
                <div className={styles.grid}>
                    {resumes.map(resume => (
                        <ResumeCard key={resume.resume_id} resume={resume} onEdit={(id) => router.push(ROUTES.DASHBOARD.PORTFOLIO.RESUME.EDIT(id))} onDelete={handleDelete} onDuplicate={handleDuplicate} onGeneratePDF={handleGeneratePDF} isLoading={isDeleting || isGenerating} />
                    ))}
                </div>
            ) : (
